...
Judge0 for run_code/list_code_runtimes and GitHub Actions for gh_run_code/
gh_get_run_result.
...
def require_run(payload: Any, label: str) -> dict[str, Any]:
    if not isinstance(payload, dict) or not isinstance(payload.get("run"), dict):
        fail(f"{label}: expected Judge0-compatible result.run object")
    return payload["run"]
...
def main() -> int:
    ...
    print(f"PASS: list_code_runtimes live Judge0 lookup ({len(runtimes)} runtimes)")
    ...
    success_payload, _ = tool_result(
        102,
        "run_code",
        {"language": "python", "code": 'print("JUDGE0_MCP_EXEC_OK")'},
    )
    success_run = require_run(success_payload, "run_code success")
    if success_run.get("code") != 0 or "JUDGE0_MCP_EXEC_OK" not in str(success_run.get("stdout", "")):
        fail(
            "run_code success: expected exit 0 and JUDGE0_MCP_EXEC_OK in stdout; "
