...
function updateLabels(){
  document.querySelectorAll('[data-provider-view-toggle="true"]').forEach(function(btn){
    setText(btn,mode==='active'?'Active providers':'All providers');
    btn.dataset.mode=mode;
    setAttr(btn,'aria-pressed',mode==='active'?'true':'false');
    setAttr(btn,'title',mode==='active'?'Showing enabled providers only. Click to show all providers.':'Showing all providers. Click to show enabled providers only.');
  });
  var summary=document.getElementById('provider-summary');
  if(summary&&totalCount())setText(summary,visibleCount()+' visible of '+totalCount());
}
function apply(){
